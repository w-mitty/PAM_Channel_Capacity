function capacity = getCapacity_update(conste, SNR, type)
    
    % conste: 'struct'; 'symbols', 'p', 'labels', 'dim'.
    ...
    % type: Coded Modulation('c') or Bit-interleaved Coded Modulation('b').
        ...
        p = conste.p; % a prior probability of constellation symbols
        % constellation points with unit average symbol energy
        symbols_norm = conste.symbols./...
            sqrt(dot(abs(conste.symbols).^2, p));
        ...
        if nargin==2||type=='c' % default case is CM
            for SNR_cnt = 1:length(SNR)
                ...
                for i = 1:M
                    sum2 = 0;
                    for j = 1:M
                        sum2 = sum2+exp(abs(xx).^2-abs(xx+symbols(i)-symbols(j)).^2)*p(j);
                    end
                    sum2 = ww.*log2(sum2);
                    sum1 = sum1+sum(sum2(:))*p(i);
                end
                capacity(SNR_cnt) = -sum1/pi;
            end
        else
            for SNR_cnt = 1:length(SNR)
